...
public extension NSManagedObjectContext {
  ...
  public func sync_safeObject(_ entityName: String, localPrimaryKey: AnyObject?, parent: NSManagedObject?, parentRelationshipName: String?) -> NSManagedObject? {
    if let localPrimaryKey = localPrimaryKey as? NSObject, entity = NSEntityDescription.entity(forEntityName: entityName, in: self) {
      let request = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
      request.predicate = Predicate(format: "%K = %@", entity.sync_localPrimaryKey(), localPrimaryKey)
      do {
      ...
      return parent?.value(forKey: parentRelationshipName) as? NSManagedObject
    } else {
      return nil
    }
  }
