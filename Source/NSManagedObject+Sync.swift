...
public extension NSManagedObject {
  ...
  func sync_fillWithDictionary(dictionary: [String : AnyObject], parent: NSManagedObject?, dataStack: DATAStack, operations: DATAFilter.Operation) {
    ...
    entity.sync_relationships().forEach { relationship in
      ...
      } else if let parent = parent where !parent.isEqual(valueForKey(relationship.name)) &&
            (relationship.destinationEntity?.name == parent.entity.name || relationship.destinationEntity?.name == parent.entity.superentity?.name) {
