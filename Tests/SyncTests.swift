...
class SyncTests: XCTestCase {
    ...
    func testObjectsForParent() {
        ...
        let user = users.first!
        XCTAssertEqual(user.valueForKey("remoteID") as? NSNumber, NSNumber(int: 6))
    ...
    func testCustomKeysInRelationshipsToMany() {
        ...
        XCTAssertEqual((user.valueForKey("notes") as? NSSet)!.allObjects.count, 3)
    ...
    func test280() {
        ...
        let routes = Helper.objectsFromJSON("280.json") as! [[String : AnyObject]]
        Sync.changes(routes, inEntityNamed: "Route", dataStack: dataStack, completion: nil)
        XCTAssertEqual(Helper.countForEntity("Route", inContext:dataStack.mainContext), 1)
    ...
    func test283() {
        ...
        let taskLists = Helper.objectsFromJSON("283.json") as! [[String : AnyObject]]
        ...
        let owner = taskList.valueForKey("owner") as? NSManagedObject
        XCTAssertNotNil(owner)

        let participants = taskList.valueForKey("participants") as? NSSet ?? NSSet()
